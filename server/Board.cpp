...
std::set<int> Group::liberties(const Board &board) const
{ // unoccupied frontiers of group
...
std::set<int> Group::antiLiberties(const Board &board) const
{ // occupied frontiers
...
std::set<int> Group::friends(const Board &board) const
{ // same color frontiers of group (ie its gotta merge)
	std::set<int> friends;
	for (int ind : stones)
	{
		std::set<int> neighbors = board.getNeighbors(ind);
...
void GroupManager::mergeGroup(int a, int b)
{ // unions two groups by their id, using lowest id for new group
...
const Group &GroupManager::getGroup(int id) const
{ // get group from id
	return groups.at(id);
}

int GroupManager::find(int a) const
{ // whats the group id of stone "a"
	for (const auto &pair : groups)
	{
		const Group &group = pair.second;
		if (group.stones.find(a) != group.stones.end())
			return group.id;
	}
	return -1;
}

std::set<int> GroupManager::getIsolated(const Board &board) const
{ // returns the ID of the isolated groups (no liberties);
	std::set<int> mySet;
	for (const auto &pair : groups)
	{
		const Group &group = pair.second;
...
void Board::removeGroupMembers(int id)
{ // helper for removeIsolated
...
// pub:
Board::Board(int s) : size(s), counter(0), board(s * s, Stone::Empty), prev(s * s, Stone::Empty) {}
...
void Board::setColor(int row, int col, Stone color)
{ // note: in gonnect, suicide is legal. NOTE: DOES NOT CHECK KO! MUST USE violatesKo SEPARATELY!!!!!!!
	prev = board;
	int i{ind(row, col)};
	if (hasStone(i))
	{
...
std::set<int> Board::getNeighbors(int ind) const
{ // returns a set of the indices of all orthogonally adjacent points
	std::set<int> neighbors;
	if (ind / size > 0)
		neighbors.insert(ind - size);
	if (ind / size < size - 1)
		neighbors.insert(ind + size);
	if (ind % size != 0)
		neighbors.insert(ind - 1);
	if ((ind + 1) % size != 0)
		neighbors.insert(ind + 1);
	return neighbors;
}
...
bool Board::isConnected(Stone color) const
{ // BFS algo to check if a certain color has a connection on the board
	// top to bottom
...
bool Board::isLegal(int row, int col, Stone color) const
{
	if (row >= size || col >= size || row < 0 || col < 0)
		return false;
	if (hasStone(ind(row, col)))
		return false;
	if (violatesKo(row, col, color))
		return false;
	return true;
}
...
void Board::printBoard() const
{
	for (int i = 0; i < size; i++)
	{
		...
		{
			int idx{ind(i, j)};
			if (board[idx] == Stone::White)
				std::cout << 'W';
			else if (board[idx] == Stone::Black)
				std::cout << 'B';
			else
				std::cout << ' ';
		}
