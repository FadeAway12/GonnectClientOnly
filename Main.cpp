...
int main() {
    ...
    CROW_ROUTE(app, "/bestMove").methods("GET"_method)([&b](const crow::request& req) { // get our best move
        auto queries = crow::query_string{ req.url_params };
    int player = 1; // if not given assume black
    if (queries.get("player")) {
        player = std::stoi(queries.get("player")); // parse as string
    }
...
crow::json::wvalue Board::boardJson() const { //will improve later.
